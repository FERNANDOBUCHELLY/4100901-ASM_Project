...
// -- Definiciones de registro para BUTTON 
    .equ RCC_BASE,       0x40021000         @ Base de RCC
    .equ RCC_AHB2ENR,    RCC_BASE + 0x4C    @ Enable GPIOC clock (AHB2ENR)
    .equ GPIOC_BASE,     0x48000800         @ Base de GPIOC
    .equ GPIOC_MODER,    GPIOC_BASE + 0x00  @ Mode register
    .equ GPIOC_ODR,      GPIOC_BASE + 0x14  @ Input data register
    .equ BUTTON_PIN          13             @ Pin del Botón
...
init_button:
    ...
    ldr   r1, [r0]
    bic   r1, r1, #(0b11 << (BUTTON_PIN * 2)) @ Limpia bits MODER5
    orr   r1, r1, #(0b01 << (BUTTON_PIN * 2)) @ PA5 como salida
    str   r1, [r0]
    bx    lr
...
init_systick:
    ...
    movs  r1, #(1 << 0)|(1 << 10)|(1 << 2)  @ ENABLE=1, TICKINT=1, CLKSOURCE=1
    str   r1, [r0]
    bx    lr
